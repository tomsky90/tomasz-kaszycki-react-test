...
class Header extends Component {
    constructor(props) {
        ...
        this.handleCloseCurrencySwitcher = this.handleCloseCurrencySwitcher.bind(this);
    }
    ...
    handleCloseCurrencySwitcher(event) {
        if (this.currencySwitcherRef && !this.currencySwitcherRef.current.contains(event.target)) {
          this.setState({
            isOptionsActive: false
          })
        } 
        ...
        return(
            ...
            <div className="header__cart-icon-currency-switcher-wrapper"> 
                <div  className="header__currency-switcher-container" >
                    <div className="currency-switcher__select-bar">{console.log(this.props)}
                        <div ref={this.currencySwitcherRef} className="currency-switcher__icon" onClick={() => {this.toggleOptionsActive()}}>
                            <span>{this.props.currencies?.selectedCurrency?.symbol }</span>
                            <img className={this.state.isOptionsActive ? 'currenccy-switcher__arrow active' : 'currenccy-switcher__arrow'} src={currencyArrow} alt=''/>
                        </div>
                        <div ref={this.cartBtnRef} onClick={() => {this.toggleBagPageActive()}} className="header__cart-icon-container" >
                            {this.props.cart.cartItems.length > 0 && <div className="header__cart-icon-container__items-in-cart">{this.totalItemsInCart()}</div>}
                    ...
                    <div className={this.state.isOptionsActive ? 'currency-switcher__options-wrapper active' : 'currency-switcher__options-wrapper'}>
                        {this.props.currencies.currencies.currencies !== undefined ? this.props.currencies.currencies.currencies.map((currency) => (
                            <div 
                                onClick={() => {this.props.setSelectedCurrency(currency); this.toggleOptionsActive()}} 
                                key={currency.label} 
                                className="option">
                                <span>{currency.symbol}</span><span>{currency.label}</span>
                            </div>
                        )) : null}
                    </div>
                </div>
            ...
            {this.state.isBagPageActive && <BagPage cartBtnRef={this.cartBtnRef} toggleBagPageActive={this.toggleBagPageActive} isBagPageActive={this.state.isBagPageActive}/>}
...
export default connect(
    (state) => ({ currencies: state.currencies, products: state.products, cart: state.cart}),
    {
      getCurrencies,
      setSelectedCurrency,
    }
